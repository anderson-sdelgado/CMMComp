...
class ReelFertigationViewModelTest {
    ...
    private val setNroEquipReel = mock<SetNroEquipReel>()
    private val viewModel = ReelFertigationViewModel(
        updateTableEquip = updateTableEquip,
        hasEquipSecondary = hasEquipSecondary,
        setNroEquipReel = setNroEquipReel
    )
    ...
    fun `setTextField - Check return failure usecase if have error in usecase CleanEquip`() =
        runTest {
            ...
            assertEquals(
                ...
                ReelFertigationState(
                    status = UpdateStatusState(
                        ...
                        flagDialog = true,
                        flagFailure = true,
                        failure = "ReelFertigationViewModel.updateAllDatabase -> CleanEquip -> java.lang.NullPointerException",
                        currentProgress = 1f,
                    )
                )
            )
            ...
            assertEquals(
                ...
                "ReelFertigationViewModel.setTextField -> ReelFertigationViewModel.updateAllDatabase -> CleanEquip -> java.lang.NullPointerException"
            )
        }
    ...
    fun `setTextField - Check return failure if field is empty`() =
        runTest {
            ...
            assertEquals(
                viewModel.uiState.value.status.failure,
                "ReelFertigationViewModel.setTextField -> ReelFertigationViewModel.updateState -> ReelFertigationViewModel.set -> Field Empty!"
            )
            assertEquals(
    ...
    fun `setTextField - Check return failure if have error in usecase HasEquipSecondary`() =
        runTest {
            ...
            assertEquals(
                viewModel.uiState.value.status.failure,
                "ReelFertigationViewModel.setTextField -> ReelFertigationViewModel.set -> IHasEquipSecondary -> java.lang.Exception"
            )
            assertEquals(
                viewModel.uiState.value.status.flagProgress,
                false
    ...
    fun `setTextField - Check return failure if have error in usecase SetNroTranshipment`() =
        runTest {
            ...
            assertEquals(
                viewModel.uiState.value.status.failure,
                "ReelFertigationViewModel.setTextField -> ReelFertigationViewModel.set -> ISetNroEquipReel -> java.lang.Exception"
            )
            assertEquals(
                viewModel.uiState.value.status.flagProgress,
                false
