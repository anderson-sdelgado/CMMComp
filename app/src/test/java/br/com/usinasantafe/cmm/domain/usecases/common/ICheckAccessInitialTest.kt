    ...
    fun `Check return failure if have error in hasConfig`() =
        runTest {
            whenever(
                ...
                Result.failure(
                    RepositoryException(
                        function = "IConfigRepository.hasConfig",
                        cause = Exception()
                    )
                )
            )
            ...
            assertEquals(
                ...
                "Failure Repository -> IConfigRepository.hasConfig"
            )
            assertEquals(
    ...
    fun `Check return failure if have error in GetFlagUpdate`() =
        runTest {
            ...
            whenever(
                ...
                Result.failure(
                    RepositoryException(
                        function = "IConfigRepository.getFlagUpdate",
                        cause = Exception()
                    )
                )
            )
            ...
            assertEquals(
                result.exceptionOrNull()!!.message,
                "Failure Repository -> IConfigRepository.getFlagUpdate"
            )
            assertEquals(
                result.exceptionOrNull()!!.cause.toString(),
                "java.lang.Exception"
