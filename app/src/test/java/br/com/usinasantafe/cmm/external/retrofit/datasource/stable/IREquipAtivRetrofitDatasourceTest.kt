...
class IREquipAtivRetrofitDatasourceTest {
    ...
    fun `Check return failure if token is invalid`() =
        runTest {
            ...
            val service = retrofit.create(REquipAtivApi::class.java)
            val datasource = IREquipAtivRetrofitDatasource(service)
            ...
            assertEquals(
                "IREquipAtivRetrofitDatasource.recoverAll",
    ...
    fun `Check return correct`() =
        runTest {
            ...
            server.start()
            server.enqueue(
                MockResponse().setBody(resultREquipAtivRetrofit)
            )
            val retrofit = provideRetrofitTest(
                server.url("").toString()
            )
...
val resultREquipAtivRetrofit = """
    [
      {"idREquipAtiv":1,"idEquip":30,"idAtiv":10},
      {"idREquipAtiv":2,"idEquip":31,"idAtiv":11}
    ]
""".trimIndent()
