package br.com.usinasantafe.cmm.external.retrofit.datasource.stable

import br.com.usinasantafe.cmm.di.external.ApiModuleTest.provideRetrofitTest
import br.com.usinasantafe.cmm.external.retrofit.api.stable.MotoMecApi
import br.com.usinasantafe.cmm.infra.models.retrofit.stable.MotoMecRetrofitModel
import kotlinx.coroutines.test.runTest
import okhttp3.mockwebserver.MockResponse
import okhttp3.mockwebserver.MockWebServer
import org.junit.Assert.*
import org.junit.Test

class IMotoMecRetrofitDatasourceTest {

    @Test
    fun `Check return failure if token is invalid`() =
        runTest {
            val server = MockWebServer()
            server.start()
            server.enqueue(
                MockResponse().setBody("{ error : Authorization header is missing }")
            )
            val retrofit = provideRetrofitTest(
                server.url("").toString()
            )
            val service = retrofit.create(MotoMecApi::class.java)
            val datasource = IMotoMecRetrofitDatasource(service)
            val result = datasource.listAll("TOKEN")

            assertEquals(
                true,
                result.isFailure
            )
            assertEquals(
                "IMotoMecRetrofitDatasource.recoverAll",
                result.exceptionOrNull()!!.message
            )
            assertEquals(
                "java.lang.IllegalStateException: Expected BEGIN_ARRAY but was BEGIN_OBJECT at line 1 column 2 path \$",
                result.exceptionOrNull()!!.cause.toString()
            )
            server.shutdown()
        }

    @Test
    fun `Check return failure if have Error 404`() =
        runTest {
            val server = MockWebServer()
            server.start()
            server.enqueue(
                MockResponse().setResponseCode(404)
            )
            val retrofit = provideRetrofitTest(
                server.url("").toString()
            )
            val service = retrofit.create(MotoMecApi::class.java)
            val datasource = IMotoMecRetrofitDatasource(service)
            val result = datasource.listAll("TOKEN")

            assertEquals(
                true,
                result.isFailure
            )
            assertEquals(
                "IMotoMecRetrofitDatasource.recoverAll",
                result.exceptionOrNull()!!.message
            )
            assertEquals(
                NullPointerException().toString(),
                result.exceptionOrNull()!!.cause.toString()
            )
            server.shutdown()
        }


    @Test
    fun `Check return correct`() =
        runTest {
            val server = MockWebServer()
            server.start()
            server.enqueue(
                MockResponse().setBody(resultMotoMecRetrofit)
            )
            val retrofit = provideRetrofitTest(
                server.url("").toString()
            )
            val service = retrofit.create(MotoMecApi::class.java)
            val datasource = IMotoMecRetrofitDatasource(service)
            val result = datasource.listAll("TOKEN")

            assertEquals(
                true,
                result.isSuccess
            )
            assertEquals(
                Result.success(
                    listOf(
                        MotoMecRetrofitModel(
                            idMotoMec = 1,
                            idOperMotoMec = 101,
                            descrOperMotoMec = "Operador A",
                            codFuncaoOperMotoMec = 1,
                            posOperMotoMec = 1,
                            tipoOperMotoMec = 1,
                            aplicOperMotoMec = 1,
                            funcaoOperMotoMec = 1
                        ),
                        MotoMecRetrofitModel(
                            idMotoMec = 2,
                            idOperMotoMec = 102,
                            descrOperMotoMec = "Operador B",
                            codFuncaoOperMotoMec = 2,
                            posOperMotoMec = 2,
                            tipoOperMotoMec = 2,
                            aplicOperMotoMec = 1,
                            funcaoOperMotoMec = 1
                        )
                    )
                ),
                result
            )
            server.shutdown()
        }
}

val resultMotoMecRetrofit = """
    [
      {"idMotoMec":1,"idOperMotoMec":101,"descrOperMotoMec":"Operador A","codFuncaoOperMotoMec":1,"posOperMotoMec":1,"tipoOperMotoMec":1,"aplicOperMotoMec":1,"funcaoOperMotoMec":1},
      {"idMotoMec":2,"idOperMotoMec":102,"descrOperMotoMec":"Operador B","codFuncaoOperMotoMec":2,"posOperMotoMec":2,"tipoOperMotoMec":2,"aplicOperMotoMec":1,"funcaoOperMotoMec":1}
    ]
""".trimIndent()
