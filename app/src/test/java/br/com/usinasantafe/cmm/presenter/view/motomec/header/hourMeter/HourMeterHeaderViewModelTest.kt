...
import br.com.usinasantafe.cmm.presenter.view.motomec.header.hourMeter.HourMeterHeaderViewModel
...
class HourMeterHeaderViewModelTest {
    ...
    fun `checkHourMeterHeader - Check return failure if have error in CheckHourMeter`() =
        runTest {
            ...
            assertEquals(
                viewModel.uiState.value.failure,
                "HourMeterHeaderViewModel.setTextField -> HourMeterHeaderViewModel.validateAndSet -> HourMeterHeaderViewModel.set -> ICheckHourMeter -> java.lang.Exception"
            )
            assertEquals(
                viewModel.uiState.value.errors,
                Errors.EXCEPTION
    ...
    fun `setHourMeterHeader - Check return failure if have error in SetHourMeterInitial`() =
        runTest {
            ...
            assertEquals(
                viewModel.uiState.value.failure,
                "HourMeterHeaderViewModel.setTextField -> HourMeterHeaderViewModel.validateAndSet -> HourMeterHeaderViewModel.set -> ISetHourMeter -> java.lang.Exception"
            )
            assertEquals(
                viewModel.uiState.value.errors,
                Errors.EXCEPTION
