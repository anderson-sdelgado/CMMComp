...
import br.com.usinasantafe.cmm.presenter.header.operator.OperatorState
...
fun ResultUpdate.resultUpdateToTurnListState(): TurnListState {
    val fail = if(failure.isNotEmpty()){
        val ret = "TurnListViewModel.updateAllDatabase -> ${this.failure}"
        Timber.e(ret)
        ret
    } else {
        this.failure
    ...
    val msg = if(failure.isNotEmpty()){
        "TurnListViewModel.updateAllDatabase -> ${this.failure}"
    } else {
        this.msgProgress
    }
    return TurnListState(
...
class TurnListViewModel @Inject constructor(
    ...
    suspend fun updateAllDatabase(): Flow<TurnListState> = flow {
        var pos = 0f
        val sizeAllUpdate = 4f
        var turnListState = TurnListState()
        updateTableTurn(
            sizeAll = sizeAllUpdate,
            count = ++pos
        ).collect {
            turnListState = it.resultUpdateToTurnListState()
            emit(
